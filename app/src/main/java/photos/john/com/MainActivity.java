...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int TAKE_PHOTOSDCARD_PERMISSION_REQUEST_CODE = 2;        // 拍照 存储权限
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonCamera.setOnClickListener( new ButtonListener());
        buttonPhoto.setOnClickListener( new ButtonListener());
    ...
    //相机权限检查
    private boolean OnCameraPermission()
    {
        boolean isPass = true;
    ...
    //相机权限检查.- 存储
    private boolean OnCameraPermissionSTORAGE()
    {
        boolean isPass = true;
        if(ContextCompat.checkSelfPermission(MainActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            // 申请读写内存卡内容的权限
            ActivityCompat.requestPermissions(MainActivity.this,
                    new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, TAKE_PHOTOSDCARD_PERMISSION_REQUEST_CODE);
        }
        return isPass;
    }

    //相册权限检查 //实际是读取内存卡的权限
    private boolean OnPhotosPermission()
    {
        boolean isPass = true;
        if(ContextCompat.checkSelfPermission(MainActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
    ...
    {
        if( !OnCameraPermission() || !OnCameraPermissionSTORAGE())
        {
            return;
        }
        ...
        if(Build.VERSION.SDK_INT >= 24) {
            photoUri = FileProvider.getUriForFile(this, this.getPackageName()+".provider", file);
        } else {
    ...
    private void cropPhoto(Uri inputUri) {
        ...
        File file = new File(getExternalFilesDir(null), "userInfo.jpg");
        //Log.d("======>",file.getPath());
//        // 设置图片的最终输出目录
...
//                photoOutputUri = Uri.parse("file:////sdcard/image_output.jpg"));
        ...
        cropPhotoIntent.putExtra("noFaceDetection", false);
        cropPhotoIntent.putExtra("scale", true);
        //裁剪框比例
        cropPhotoIntent.putExtra("aspectX", 1);
        cropPhotoIntent.putExtra("aspectY", 1);
        //输出图片大小
        cropPhotoIntent.putExtra("outputX", 512);
        cropPhotoIntent.putExtra("outputY", 512);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        switch (requestCode) {
            ...
            case TAKE_PHOTO_PERMISSION_REQUEST_CODE:
                ...
                break;
            // 调用相机拍照： 存储
            case  TAKE_PHOTOSDCARD_PERMISSION_REQUEST_CODE:
                // 如果用户授予权限，那么打开相机拍照
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    OnCamera();
                } else {
